extends Control #MarginContainer

#export (NodePath) var FARMER  # Link to Player node. If this is null, the map will not function.
#export var zoom = 1.5 setget set_zoom # Scale multiplier.

# Node references.
#onready var grid = $MarginContainer/Grid
#onready var farmer_marker = $MarginContainer/Grid/Farmer
#onready var cow_marker = $MarginContainer/Grid/Cow
#onready var sheep_marker = $MarginContainer/Grid/Sheep
# Link object icon setting to Sprite marker.
#onready var icons = {"cow": cow_marker, "sheep": sheep_marker}
...
#	farmer_marker.position = grid.rect_size / 2
...
#		return
	...
	#farmer_marker.rotation = get_node(FARMER).rotation + PI/2
#	for item in markers:
#		var obj_pos = (item.position - get_node(FARMER).position) * grid_scale + grid.rect_size / 2
...
#			markers[item].hide()
		...
		#obj_pos.z = clamp(obj_pos.z, 0, grid.rect_size.z)
